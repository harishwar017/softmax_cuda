...
__global__ void addition_kernel(const float* input, float* deno, int N, float global_max) {
    ...
    if (index < N) {
        float v = expf(input[index] - global_max);
        // Atomically add each thread's contribution to the global denominator.
        atomicAdd(deno, v);
    }
}
...
extern "C" void solve(const float* input, float* output, int N) {
    ...
    float ms; // For storing temporary kernel time
    ...
    // KERNEL 2: Addition (Compute Denominator for Softmax)
    cudaEventRecord(start, 0);
