...
__global__ void norm_kernel(const float* input, float* maxi_p, int N){
    ...
    if (thread < 32) {
        // Continue Shared Memory reduction until i=32
        if (thread < 32 && blockDim.x > 32) {
            temp[thread] = fmaxf(temp[thread], temp[thread + 32]);
        }
        // We can now use shuffles to speed up the final 32-element reduction.
        // This mask specifies all threads in the current warp are participating
        unsigned int mask = 0xffffffff; 

        // i=16, 8, 4, 2, 1 steps using shuffle
        for (int i = 16; i > 0; i /= 2) {
            float value = __shfl_down_sync(mask, temp[thread], i);
            temp[thread] = fmaxf(temp[thread], value);
        }
    }
...
__global__ void addition_kernel(const float* input, float* deno, int N, float global_max) {
    ...
    // Block-Level Reduction (Shared Memory) - Stop when i <= 32
    for(int i= blockDim.x/2; i > 32; i/=2){
        if(thread < i){
